...
public class GalleryModel {
	...
	protected void init() {
		...
		try {
			if (resource != null && !resource.getPath().contains("/conf/")) {
					...
					homePagePath=PlayHelper.getHomePagePath(resource);
	...
	private String checkLandingGridPath() {
		...
		if ("characters".equals(galleryFor)) {
			...
			landinggridPath = PlayHelper.checkProductThumbnailExpFragemnt(resolver, productPageRootRes);
		}
	...
	public String getCtaButtonLink() {
		if (null != linkTitle) {
			return PlayHelper.checkLink(ctaButtonLink,resource) + '#' + linkTitle.toLowerCase();
		} else {
			return PlayHelper.checkLink(ctaButtonLink,resource);
		}
