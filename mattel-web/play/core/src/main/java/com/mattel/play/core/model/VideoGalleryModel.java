...
public class VideoGalleryModel {
	...
	public String getVideoUrl() {
		...
		videoUrl = PropertyReaderUtils.getPlayPath() + PlayHelper.getBrandName(resource.getPath())
				+ PlayHelper.getRelativePath(resource.getPath(), resource) + PropertyReaderUtils.getHomePath()
				+ PropertyReaderUtils.getVideoPath();

		videoUrl = PlayHelper.checkLink(videoUrl, resource) + Constants.VIDEO_URL;
	...
	public List<VideoTile> getVideoListLanding() {
		...
		try {
			if (null != resource && !resource.getPath().contains("/conf/")) {
				String videoNodePath = PropertyReaderUtils.getPlayPath() + PlayHelper.getBrandName(resource.getPath())
						+ PlayHelper.getRelativePath(resource.getPath(), resource) + PropertyReaderUtils.getHomePath()
						+ PropertyReaderUtils.getVideoPath() + Constants.JCR_CONTENT_ROOT;
	 ...
	 * @return
	 * Method to Fetch Video Details by Date
	...
	private String getVideoSourcePathDetails(String pageLocale) {
		...
		if (StringUtils.isNotEmpty(videoAssetPath)) {
			...
			path = PropertyReaderUtils.getPlayDamPath() + PlayHelper.getBrandName(resource.getPath())
	...
	public List<VideoTile> checkVideoUrl(List<VideoTile> videoList, Resource resource) {
		String currentBrandName = PlayHelper.getBrandName(resource.getPath());
		if (currentBrandName.equals(PlaySiteConfigurationUtils.getFisherPriceKidsBrandName())) {
			for (VideoTile videoTile : videoList) {
				String path = videoTile.getVideoThumbnail();
				path=path.substring(0,path.lastIndexOf('/'));
				path = path.replace(PropertyReaderUtils.getPlayDamPath(), PropertyReaderUtils.getPlayPath());
				
					path = path.replace(
							PropertyReaderUtils.getVideoDamPath()
									+ PlayHelper.fetchLocaleFromDam(videoTile.getVideoThumbnail(), resource),
							PlayHelper.getRelativePath(resource.getPath(), resource));
				videoTile.setVideoUrl(PlayHelper.checkLink(path,resource) + Constants.VIDEO_URL);
			}
