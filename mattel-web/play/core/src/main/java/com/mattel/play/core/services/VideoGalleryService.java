...
import com.mattel.play.core.pojos.VideoCategory;
...
public class VideoGalleryService {
	...
	List<VideoTile> masterListForVideoTile;
	 ...
	 *            * @return
	 *
	 *            Method used for Manual and Landing option in Gallery.
	 */
	...
	public List<VideoTile> getVideosByDate(String path, String selectedTags, boolean isGallery) {
		...
		if (resolverFactory != null) {
			try {
				...
				String orderbyProp = PlaySiteConfigurationUtils.getVideoSortOrderProperty();
	...
	private VideoTile buildNode(Node videoTileNode, ResourceResolver resolver) {
		...
		try {
			if (videoTileNode != null) {
				...
				videoThumbnailPath = PlayHelper.checkForProperty(videoTileNode, "videoThumbnail");
	 ...
	 * @return List of void
	 */
	private List<VideoTile> buildMasterList(List<VideoTile> nodesList, List<VideoTile> nodeList2,
											List<VideoTile> nodeList3, List<VideoTile> nodeList4) {
		LOGGER.info("buildMasterList -> Start");
		masterListForVideoTile = new ArrayList<>();
		int count = 0;
		for (int i = 0; i < nodesList.size(); i++) {
			if (null != nodesList.get(i)) {
				masterListForVideoTile.add(nodesList.get(i));
				if (nodeList2.size() > count) {
					masterListForVideoTile.add(nodeList2.get(i));
				}
				if (nodeList3.size() > count) {
					masterListForVideoTile.add(nodeList3.get(i));
				}
				if (!nodeList4.isEmpty() && nodeList4.size() > count) {
					masterListForVideoTile.add(nodeList4.get(i));
				}
			}
			count++;
		}
		LOGGER.info("buildMasterList -> End");
		return masterListForVideoTile;
	}
		...
		if (orderVideoBy.equals(manual)) {
			...
			if (StringUtils.isNotEmpty(videoAssetPath)) {
				...
				if (videoGalleryNodepath.contains("/language-masters/")) {
					...
					pageLocale = PlayHelper.getPageLocale(videoGalleryNodepath);
				}
				path = PropertyReaderUtils.getPlayDamPath() + PlayHelper.getBrandName(videoGalleryNodepath)
	...
	private VideoCategory fetchCategorySeo(Node categoryNode) throws RepositoryException {
		...
		if (categoryNode != null) {
			String metaKeys = "";
			String seoTitle = PlayHelper.checkForProperty(categoryNode, "seoTitle");
			if (seoTitle.isEmpty()) {
				seoTitle = PlayHelper.checkForProperty(categoryNode, "globalSeoTitle");
			}
			catPojo.setSeotitle(seoTitle);
			String seoUrl = PlayHelper.checkForProperty(categoryNode, "canonicalUrl");
			catPojo.setSeoUrl(seoUrl);
			String metaDesc = PlayHelper.checkForProperty(categoryNode, "metaDescription");
