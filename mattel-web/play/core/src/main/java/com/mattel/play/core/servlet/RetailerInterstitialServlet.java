...
public class RetailerInterstitialServlet extends SlingSafeMethodsServlet {
	...
	transient Resource retailerResource;
	...
	private void fetchRetailNode(String homePagePath, String pagePath, SlingHttpServletRequest request) {
		...
		if (homePagePath.equals(tempPath)) {
			retailerResource = request.getResourceResolver()
					.getResource(pagePath + com.mattel.play.core.constants.Constants.RETAILER_NODE);
		} else {
			retailerResource = request.getResourceResolver()
					.getResource(pagePath + com.mattel.play.core.constants.Constants.SLASH_JCR_CONTENT
							+ com.mattel.play.core.constants.Constants.PRODUCT_RETAILER_NODE);
		}
