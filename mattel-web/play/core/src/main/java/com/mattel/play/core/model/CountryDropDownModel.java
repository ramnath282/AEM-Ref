...
import com.mattel.play.core.pojos.CountryDropDownPojo;
...
public class CountryDropDownModel {
    ...
    protected void init() {
        ...
        if (null != prop) {
            ...
            String rootPagePath = "";
            if (PlayHelper.checkIfSiteisMattelPlay(pagePath)) {
                rootPagePath = PropertyReaderUtils.getPlayPath() + PlayHelper.getBrandName(pagePath);
            } else {
            ...
            SearchResult result = PlayHelper.getCountryNodesByLanguage(rootPagePath, session, queryBuilder);
            if (null != result) {
                try {
    ...
    private void getCountryDetails(SearchResult result, Resource resource, String targetValue)
        ...
        for (Hit hit : result.getHits()) {
            if (null != hit.getPath()) {
				...
				if (null != pgmgr) {
					generateCountryDetails (resource, pgmgr, targetValue, hit);
				}
            }
        }
	 ...
	 * @throws RepositoryException
	 */
	private void generateCountryDetails(Resource resource, PageManager pgmgr, String targetValue, Hit hit) throws RepositoryException {
		LOGGER.info("generateCountryDetails method of CountryDropDownModel --> Start");
		String currentPageCountry = fetchCountryPath(pagePath);
		LOGGER.debug("currentPageCountry is {}", currentPageCountry);
		...
		LOGGER.debug("pageTitle is {}", pageTitle);
            ...
            String tempPath = PlayHelper.checkLink(pagepath + "home", resource);
		LOGGER.debug("tempPath is {}", tempPath);
		String redirect = PlayHelper.checkLink(page.getProperties().get("cq:redirectTarget", String.class),	resource);
		LOGGER.debug("redirect path is {}", redirect);
		...
		LOGGER.debug("optionCountry is {}", optionCountry);
		...
		LOGGER.info("generateCountryDetails method of CountryDropDownModel --> End");
        }
    ...
    private String fetchCountryPath(String currentPagePath) {
		...
		if(Objects.nonNull(currentPagePath) && !currentPagePath.contains("/experience-fragments/")) {
