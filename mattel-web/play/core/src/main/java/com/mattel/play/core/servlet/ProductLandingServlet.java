		...
		if (tagLength != 0 || productDetailPage.equalsIgnoreCase(pageResType)) {
			...
			if (null != parentPage && StringUtils.isNoneEmpty(parentPage.getPath())) {
				Resource currPageRootRes = resolver.getResource(
						parentPage.getPath() + com.mattel.play.core.constants.Constants.JCR_CONTENT_ROOT_WOSLASH);
				productThumbnailGridNode = PlayHelper.checkProductThumbnailExpFragemnt(resolver, currPageRootRes);
			}
		} else {
			Resource currPageRootRes = resolver
					.getResource(currentPath + com.mattel.play.core.constants.Constants.JCR_CONTENT_ROOT_WOSLASH);
			productThumbnailGridNode = PlayHelper.checkProductThumbnailExpFragemnt(resolver, currPageRootRes);
		}
