...
import com.mattel.play.core.pojos.ProductAsset;
...
public class ProductGalleryAndLandingService {
	...
	public List<ProductTilePojo> getTilesByDate(String parentPath, boolean linktoPages) {
		...
		try {
			...
			if (null != pageQuery) {
				...
				masterList.clear();
				iterateSearchResult(linktoPages, result);
	...
	/**
	 * iterate search result for getTilesByDate and call
	 * fetchProductDetailsByProductPath
	 *
	 * @param linktoPages
	 * @param result
	 *            query search result
	 * @throws RepositoryException
	 */
	private void iterateSearchResult(boolean linktoPages, SearchResult result) throws RepositoryException {
		if (null != result) {
			for (Hit hit : result.getHits()) {
				if (null != resolver.getResource(hit.getPath())) {
					...
					if (null != productResource) {
						...
						productTilePojo = prepareProductPojo(productResource);
						String productPath = valMap.get("productPath", String.class);
						fetchProductDetailsByProductPath(productPath, "", hit, "byDate", productTilePojo, linktoPages,
								resolver);
					}
				}
			}
		...
		try {
			...
			if (pageManager != null) {
				...
				if (null != productResource && null != productPage) {
					...
					if (linktoPages) {
						productTilePojo.setProductPagePath(PlayHelper.checkLink(productPageURL,productResource));
					}
	...
	/**
	 * @param tags
	 *            product page tag array
	 * @param pageTags
	 *            String list to told all tags
	 */
	private void getAllPageTags(Tag[] tags, List<String> pageTags) {
		if (null != tags) {
			...
			if (productNode.hasNode("assets")) {
				...
				if (null != assetsNode) {
					...
					for (Map.Entry<String, ValueMap> entry : multifieldProperty.entrySet()) {
						...
						String tileImageAltText = PlayHelper.getAssetMetadataValue(tileImage, resolver,
								"dc:imageAltText");
	...
	private void fetchAssetIsPrimary(Object[] tags, TagManager tagmanager, ProductTilePojo productTilePojo,
		...
		for (Object obj : tags) {
			...
			if (null != tag) {
				...
				if (PlaySiteConfigurationUtils.getProductPrimaryImageTag().equalsIgnoreCase(tagName)) {
	...
	/**
	 * get the experience fragment rootPath/site root Path  based on condition
	 * @param currentBrandName
	 * @return
	 */
	public String getCurrentBrandExpFragmentRootPath(String currentBrandName, String expOrSiteRootPath, String resourcePath) {
		LOGGER.info("getCurrentBrandExpFragmentRootPath() starts here ");
		if(StringUtils.isNotBlank(expOrSiteRootPath)) {
			if (expOrSiteRootPath.equals("experience-fragments")) {
				String[] brandRootPath = PlaySiteConfigurationUtils.getExpFragmentRootPathArray();
				return getExpFragOrBrandSiteRootPath(brandRootPath, currentBrandName, resourcePath);
			}else {
				String[] brandRootPath = PlaySiteConfigurationUtils.getBrandSiteRootPathArray();
				return getExpFragOrBrandSiteRootPath(brandRootPath, currentBrandName, resourcePath);
			}
	...
	private String getExpFragOrBrandSiteRootPath(String[] brandRootPath,String currentBrandName, String resourcePath) {
