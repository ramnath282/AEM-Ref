export default class fieldValidation {
    ...
    isPhoneNumber(num) {
        ...
        let isValidPhone = phoneNumber.test(num);
        if(isValidPhone){
    ...
    shouldBeValidated(field, validationForOptionalFields) {
        let $elm = $(field),
        ...
        if (typeof $requiredElem !== typeof undefined && $requiredElem !== false) {
            ...
            return false;
        } else if(validationForOptionalFields){
            if(field.value == "") $elm.closest('.input-field').removeClass('has-error');
            else this.getErrorMessage(field);
        }
        return true;
    }
        ...
        if(!$("#emailLightBoxModal").is(":visible")){
            switch (field.name || field.dataset.attr) {
                ...
                case 'giftCardamount':
                    ...
                    break;
                case String(field.name.match(/^gender.*/)):
                    errorName = !$(field.form).find(`input[name=${field.name}]`).is(':checked')  && 'ERROR_GENDER_FIELDREQUIRED';
                    break;
                case "guardianOption":
                case "relationship":  
                    errorName = !$(field.form).find(`input[name=${field.name}]`).is(':checked')  && 'ERROR_RELATIONSHIP_FIELDREQUIRED';
                    break;
                default:
                    if(field.type == "checkbox"){
                        errorName = !$(field).is(":checked") ? 'ERROR_FIELDREQUIRED' : '';
                    } else{
    ...
    instantValidation(field, checkOnlyValidation) {
        if (this.shouldBeValidated(field, checkOnlyValidation)) {
            let invalid =
    ...
    checkAllFields(form, cb) {
        let isFormError;
        let $formEle = $(form);
        let formInputs = $formEle[0].querySelectorAll('select,input:not([type="hidden"]):not(.hide):not(:disabled),textarea');
        ...
        if (isFormError) {
            $formEle.find('.has-error').first().find('input,select,textarea').focus();
            cb(false, formInputs);
            return;
        }
        cb(true);
