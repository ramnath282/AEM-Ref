...
(function($, ns, channel, window) {
    ...
    var NS = ".cmp-childreneditor_content";
    var NN_PREFIX = "content_";
    var PN_PANEL_TITLE = "cq:panelTitle";
    var PN_RESOURCE_TYPE = "sling:resourceType";
    var POST_SUFFIX = ".container.html";

    var selectors_content = {
        self: "[data-cmp-is='childreneditor_content']",
        add: "[data-cmp-hook-childreneditor_content='add']",
        insertComponentDialog: {
            self: "coral-dialog.InsertComponentDialog",
            selectList: "coral-selectlist"
        },
        item: {
            icon: "[data-cmp-hook-childreneditor_content='itemIcon']",
            input: "[data-cmp-hook-childreneditor_content='itemTitle']",
            hiddenInput: "[data-cmp-hook-childreneditor_content='itemResourceType']"
        }
    };

    /**
     * @typedef {Object} childreneditor_contentConfig Represents a Children Editor configuration object
     ...
     * @param {childreneditor_contentConfig} config The Children Editor configuration object
     */
    var childreneditor_content = function(config) {
        this._config = config;
        this._elements = {};
        this._path = "";
        ...
        $(window).adaptTo("foundation-registry").register("foundation.adapters", {
            type: "cmp-childreneditor_content",
            selector: selectors_content.self,
            adapter: function() {
                return {
                    items: function() {
                        var items = [];
                        that._elements.self.items.getAll().forEach(function(item) {
                            var component = item.querySelector(selectors_content.item.icon + " [title]").getAttribute("title");
                            var title = item.querySelector(selectors_content.item.input);
    ...
    childreneditor_content.prototype = (function() {

        return {

            constructor: childreneditor_content,
            ...
            _init: function() {
                ...
                this._elements.add = this._elements.self.querySelectorAll(selectors_content.add)[0];
                this._path = this._elements.self.dataset["containerPath"];

                // store a reference to the Children Editor object
                $(this._elements.self).data("childreneditor_content", this);
            ...
            _renderIcon: function(component) {
                ...
                if (iconName) {
                    ...
                    iconHTML.classList.add("cmp-childreneditor_content__item-tag");
                }
                    ...
                    Coral.commons.ready(that._elements.add, function() {
                        that._elements.add.on("click", function() {
                            ...
                            if (children.length > 0) {
                                ...
                                var insertComponentDialog = $(document).find(selectors_content.insertComponentDialog.self)[0];
                                var selectList = insertComponentDialog.querySelectorAll(selectors_content.insertComponentDialog.selectList)[0];
                                    ...
                                    selectList.on("coral-selectlist:change" + NS, function(event) {
                                        ...
                                        if (components.length > 0) {
                                            ...
                                            Coral.commons.nextFrame(function() {
                                                ...
                                                var input = item.querySelectorAll(selectors_content.item.input)[0];
                                                input.name = "./" + name + "/" + PN_PANEL_TITLE;
                                                input.placeholder = Granite.I18n.get(componentTitle);

                                                var hiddenInput = item.querySelectorAll(selectors_content.item.hiddenInput)[0];
                                                hiddenInput.value = resourceType;
                                                hiddenInput.name = "./" + name + "/" + PN_RESOURCE_TYPE;

                                                var itemIcon = item.querySelectorAll(selectors_content.item.icon)[0];
    ...
    channel.on("foundation-contentloaded", function(event) {
        $(event.target).find(selectors_content.self).each(function() {
            new childreneditor_content({
                el: this
            });
        });
    });
    ...
    $(window).adaptTo("foundation-registry").register("foundation.form.submit", {
        ...
        handler: function(form) {
            ...
            var el = form.querySelectorAll(selectors_content.self)[0];
            var childreneditor_content = $(el).data("childreneditor_content");
            if (childreneditor_content) {
                return {
                    post: function() {
                        return childreneditor_content.update();
                    }
                };
            } else {
                return {};
