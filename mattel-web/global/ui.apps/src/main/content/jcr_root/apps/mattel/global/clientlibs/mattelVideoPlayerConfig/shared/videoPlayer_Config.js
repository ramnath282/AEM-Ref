...
const config = {
    ...
    isManualTrigger: true, //$("body").hasClass("basicpage") ? false : true,
...
deluxePlayer.prototype = {
    ...
    loadCSSFile: () => {
        $('head').append(`<link rel="stylesheet" type="text/css" href="${self.cssUrl}">`);
    },
    isScrolledIntoView: (elem) => {
        var elementTop = $(elem).offset().top;
        var elementBottom = elementTop + $(elem).outerHeight();
    ...
    init: (cb,skipErrorCond) => {
        if ((Object.keys(config.error).length || !$(config.el).length) && !skipErrorCond) {
            typeof cb == "function" && cb(false);
            return;
        }
        self.loadCSSFile();
...
const deluxeInit = playerConfigs['init'];
