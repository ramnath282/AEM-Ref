...
import com.mattel.ecomm.productdetails.core.servicesimpl.DHAddToCartServiceImpl;

import okhttp3.mockwebserver.MockResponse;
import okhttp3.mockwebserver.MockWebServer;

@RunWith(MockitoJUnitRunner.class)
public class DHAddToCartServiceImplTest {

    @InjectMocks
    DHAddToCartServiceImpl dhAddToCartServiceImpl;
    @Mock
    PropertyReaderService propertyReaderService;

    private final String endPointUrl = "http://localhost:PORT_NUMBER/wcs/resources/store/STORE_ID/mattelProductPrice/getProductPrice?responseFormat=json&updateCookies=true&storeId=STORE_ID";
    private MockWebServer mockWebServer;

    @Test
    public void testDHAddToCartService() throws ServiceException, IOException {
        try (InputStream is = getClass().getResourceAsStream("add_to_cart_response.json");
                InputStream is1 = getClass().getResourceAsStream("get_add_to_cart_request.json")) {
            final DHAddToCartServiceImpl.Config config = Mockito.mock(DHAddToCartServiceImpl.Config.class);
            ...
            dhAddToCartServiceImpl.activate(config);
            DHAddCartResponse dhAddCartResponse = dhAddToCartServiceImpl.addToCart(requestMap);
            Assert.assertNotNull(dhAddCartResponse);
        } finally {
    ...
    public void testDHAddToCartServiceWithWCSError() throws IOException, ServiceException {
        try (InputStream is1 = getClass().getResourceAsStream("get_add_to_cart_request.json")) {
            final DHAddToCartServiceImpl.Config config = Mockito.mock(DHAddToCartServiceImpl.Config.class);
            final MockResponse mockResponse = new MockResponse();
            final Map<String, Object> requestMap = new HashMap<>();
            final Cookie cookie = new Cookie("JSESSIONID", "213123132");
            ...
            Mockito.when(propertyReaderService.getCookieDomain("AG")).thenReturn("localhost");
            dhAddToCartServiceImpl.activate(config);
            dhAddToCartServiceImpl.addToCart(requestMap);
    ...
    @Test(expected = ServiceException.class)
    public void testDHAddToCartServiceWithWCSDown() throws IOException, ServiceException {
        try (InputStream is1 = getClass().getResourceAsStream("get_add_to_cart_request.json")) {
            final DHAddToCartServiceImpl.Config config = Mockito.mock(DHAddToCartServiceImpl.Config.class);
            final MockResponse mockResponse = new MockResponse();
            final Map<String, Object> requestMap = new HashMap<>();
