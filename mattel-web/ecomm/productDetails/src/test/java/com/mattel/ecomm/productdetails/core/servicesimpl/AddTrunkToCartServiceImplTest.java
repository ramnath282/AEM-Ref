...
public class AddTrunkToCartServiceImplTest {

	 private static final String ENDPOINT_URL = "http://localhost:PORT_NUMBER/wcs/resources/store/STORE_ID/xcart/@self/mattel_add_configuration_to_cart?storeId=STORE_ID&responseFormat=json&updateCookies=true";
	  @Mock
	  private PropertyReaderService propertyReaderService;
	  @InjectMocks
	  private AddTrunkToCartServiceImpl impl;
	  private MockWebServer mockWebServer;
	  
	  @Test
	  public void testAddToCart() throws IOException, ServiceException {
	    try (InputStream is1 = getClass().getResourceAsStream("add_trunk_to_cart_request.json");
	        InputStream is2 = getClass().getResourceAsStream("add_trunk_to_cart_response.json")) {
	      final AddTrunkToCartServiceImpl.Config config = Mockito
	          .mock(AddTrunkToCartServiceImpl.Config.class);
	      final MockResponse mockResponse = new MockResponse();
	      final Map<String, Object> requestMap = new HashMap<>();
	      final Cookie cookie = new Cookie("JSESSIONID", "213123132");
	      final Cookie[] cookieObj = new Cookie[1];
	      TrunkCartResponse trunkCartResponse;
	      ...
	      Mockito.when(config.endPoint()).thenReturn(AddTrunkToCartServiceImplTest.ENDPOINT_URL
	      ...
	      trunkCartResponse = impl.addToCart(requestMap);
	      Assert.assertNotNull(trunkCartResponse);
	      Assert.assertNotNull(trunkCartResponse.getOrderId());
	      Assert.assertNotNull(trunkCartResponse.getOrderItemId());
	    } finally {
