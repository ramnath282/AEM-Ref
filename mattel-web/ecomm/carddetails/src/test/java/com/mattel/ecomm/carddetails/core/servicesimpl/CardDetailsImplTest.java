...
public class CardDetailsImplTest {

    private final String endPointUrl = "http://localhost:PORT_NUMBER/wcs/resources/store/STORE_ID/xcreditcard/xCreditCard?updateCookies=true&storeId=STORE_ID&creditCardId=CREDITCARD_ID&profileName=xCreditCard&userId=USER_ID&responseFormat=json";
    ...
    @Test
    public void testcarddetails() throws IOException, ServiceException {
        try (InputStream is = getClass().getResourceAsStream("carddetails_response.json")) {
            final CardDetailsImpl.Config config = Mockito.mock(CardDetailsImpl.Config.class);
            ...
            requestMap.put("content-encoding", Constant.CONTENT_TYPE_APPLICATION_JSON);
            requestMap.put(Constant.CREDITCARD_ID, "10458722CTBkL");
            requestMap.put(Constant.REQUEST_USERID, "11323722");
            ...
            cardDetailsImpl.activate(config);
            cardDetailsResponse = cardDetailsImpl.getCardDetails(requestMap);
            Assert.assertNotNull(cardDetailsResponse);
            Assert.assertEquals("address", cardDetailsResponse.getAddressId());
            Assert.assertEquals("xxxx", cardDetailsResponse.getCardName());
            Assert.assertEquals("Gift Card", cardDetailsResponse.getCardType());
            Assert.assertEquals("May 6,2019", cardDetailsResponse.getCreateDate());
            Assert.assertEquals("10458722CTBkL", cardDetailsResponse.getCreditCardid());
            Assert.assertEquals("tag", cardDetailsResponse.getDefaultTag());
            Assert.assertEquals("June", cardDetailsResponse.getExpMonth());
            Assert.assertEquals("2020", cardDetailsResponse.getExpYear());
            ...
            Assert.assertEquals("1544610", cardDetailsResponse.getHashCode());
            Assert.assertEquals("xx", cardDetailsResponse.getLastUser());
            Assert.assertEquals("mAccount", cardDetailsResponse.getMaskAccount());
            Assert.assertEquals("11323722", cardDetailsResponse.getUsersId());
            Assert.assertEquals("1544610620251-5", cardDetailsResponse.getToken());
        } finally {
    ...
    public void testCardDetailsForWCSError() throws IOException, ServiceException {
        try {
            final CardDetailsImpl.Config config = Mockito.mock(CardDetailsImpl.Config.class);
            final MockResponse mockResponse = new MockResponse();
            final Map<String, Object> requestMap = new HashMap<>();
    ...
    @Test(expected = ServiceException.class)
    public void testCardDetailsForWCSDown() throws IOException, ServiceException {
        try {
            final CardDetailsImpl.Config config = Mockito.mock(CardDetailsImpl.Config.class);
            final MockResponse mockResponse = new MockResponse();
            final Map<String, Object> requestMap = new HashMap<>();
            ...
            Mockito.when(propertyReaderService.getCookieDomain("AG")).thenReturn("localhost");
            cardDetailsImpl.activate(config);
            cardDetailsImpl.getCardDetails(requestMap);
        } catch (final ServiceException serviceException) {
