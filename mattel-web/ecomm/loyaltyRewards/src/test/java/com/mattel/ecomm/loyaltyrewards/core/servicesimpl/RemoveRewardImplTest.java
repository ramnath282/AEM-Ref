...
public class RemoveRewardImplTest {

  private final String endPointUrl = "http://localhost:PORT_NUMBER/wcs/resources/store/STORE_ID/cart/@self/assigned_promotion_code/PROMO_ID?responseFormat=json&updateCookies=true";

  private MockWebServer mockWebServer;

  @InjectMocks
  private RemoveRewardImpl removeRewardImpl;

  @Mock
  PropertyReaderService propertyReaderService;

  @Test
  public void testRemoveReward() throws IOException, ServiceException {

    try (InputStream is1 = getClass().getResourceAsStream("remove_promo_request.json");
        InputStream is2 = getClass().getResourceAsStream("remove_promo_response.json");) {
      final RemoveRewardImpl.Config config = Mockito.mock(RemoveRewardImpl.Config.class);
      ...
      removeRewardImpl.activate(config);
      RemoveRewardResponse removeRewardResponse = removeRewardImpl.remove(requestMap);
      Assert.assertNotNull(removeRewardResponse);
    } finally {
  ...
  public void testLoyaltyRewardsWithWCSError() throws IOException, ServiceException {
    try (InputStream is1 = getClass()
        .getResourceAsStream("remove_promo_request.json")) {
      final RemoveRewardImpl.Config config = Mockito
          .mock(RemoveRewardImpl.Config.class);
      final MockResponse mockResponse = new MockResponse();
      final Map<String, Object> requestMap = new HashMap<>();
      final Cookie cookie = new Cookie("JSESSIONID", "213123132");
      final Cookie[] cookieObj = new Cookie[1];
      ...
      Mockito.when(propertyReaderService.getCookieDomain("AG")).thenReturn("localhost");
      removeRewardImpl.activate(config);
      removeRewardImpl.remove(requestMap);
  ...
  @Test(expected = ServiceException.class)
  public void testLoyaltyRewardsWithWCSDown() throws IOException, ServiceException {
    try (InputStream is1 = getClass()
        .getResourceAsStream("remove_promo_request.json")) {
      final RemoveRewardImpl.Config config = Mockito
          .mock(RemoveRewardImpl.Config.class);
