...
public class ApplyRewardsImplTest {
  private final String endPointUrl = "http://localhost:PORT_NUMBER/wcs/resources/store/STORE_ID/cart/@self/assigned_promotion_code?responseFormat=json&updateCookies=true";
  ...
  @InjectMocks
  private ApplyRewardsImpl applyRewardsImpl;

  @Test
  public void testApplyReward() throws IOException, ServiceException {
    try (InputStream is1 = getClass().getResourceAsStream("apply_rewards_request.json");
        InputStream is2 = getClass().getResourceAsStream("apply_rewards_response.json")) {
      final ApplyRewardsImpl.Config config = Mockito.mock(ApplyRewardsImpl.Config.class);
      ...
      applyRewardsImpl.activate(config);
      applyRewardsResponse = applyRewardsImpl.applyReward(requestMap);
      Assert.assertNotNull(applyRewardsResponse);
      Assert.assertEquals("11001",applyRewardsResponse.getOrderId());
    }finally {
  ...
  public void testApplyReward_withWCSError() throws IOException, ServiceException {
      try (InputStream is1 = getClass().getResourceAsStream("apply_rewards_request.json")) {
          final ApplyRewardsImpl.Config config = Mockito.mock(ApplyRewardsImpl.Config.class);
          final MockResponse mockResponse = new MockResponse();
          final Map<String, Object> requestMap = new HashMap<>();
          ...
          Mockito.when(propertyReaderService.getCookieDomain("AG")).thenReturn("localhost");
          applyRewardsImpl.activate(config);
          applyRewardsImpl.applyReward(requestMap);
  ...
  @Test(expected = ServiceException.class)
  public void testApplyReward_withWCSDown() throws IOException, ServiceException {
      try (InputStream is1 = getClass().getResourceAsStream("apply_rewards_request.json")) {
          final ApplyRewardsImpl.Config config = Mockito.mock(ApplyRewardsImpl.Config.class);
          final MockResponse mockResponse = new MockResponse();
          final Map<String, Object> requestMap = new HashMap<>();
  ...
  @Test(expected = ServiceException.class)
  public void testApplyReward_withEmptyRequestPayload() throws IOException, ServiceException {
      try {
          final ApplyRewardsImpl.Config config = Mockito.mock(ApplyRewardsImpl.Config.class);
          final MockResponse mockResponse = new MockResponse();
          final Map<String, Object> requestMap = new HashMap<>();
