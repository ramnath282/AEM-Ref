...
public class AddNewChildInfoImplTest {
    ...
    @InjectMocks
    private AddNewChildInfoImpl impl;

    @Test
    public void testAddChildInfo() throws Exception {
        try (InputStream is1 = getClass().getResourceAsStream("add_new_child_request.json")) {
            final AddNewChildInfoImpl.Config config = Mockito.mock(AddNewChildInfoImpl.Config.class);
            ...
            mockResponse.setBody("{\"viewTaskName\":\"RedirectView\",\"addressId\":\"25540533\"}");
            ...
            baseResponse = impl.addChildInfo(requestMap);
    ...
    public void testAddChildInfoForWCSDown() throws Exception {
        try (InputStream is1 = getClass().getResourceAsStream("add_new_child_request.json")) {
            final AddNewChildInfoImpl.Config config = Mockito.mock(AddNewChildInfoImpl.Config.class);
            final Map<String, Object> requestMap = new HashMap<>();
            final Cookie cookie = new Cookie("JSESSIONID", "213123132");
            ...
            requestMap.put(Constant.REQUEST_COOKIES_KEY, cookieObj);
            Mockito.when(config.endPoint())
                    .thenReturn(endPointUrl.replace("PORT_NUMBER", String.valueOf("8090")));
            Mockito.when(propertyReaderService.getStoreId("AG")).thenReturn("10651");
            Mockito.when(propertyReaderService.getCookieDomain("AG")).thenReturn("localhost");
            impl.activate(config);
            impl.addChildInfo(requestMap);
    ...
    @Test(expected = ServiceException.class)
    public void testAddChildInfoForWCSError() throws Exception {
        try (InputStream is1 = getClass().getResourceAsStream("add_new_child_request.json")) {
            final AddNewChildInfoImpl.Config config = Mockito.mock(AddNewChildInfoImpl.Config.class);
            final MockResponse mockResponse = new MockResponse();
            final Map<String, Object> requestMap = new HashMap<>();
    ...
    @Test(expected = ServiceException.class)
    public void testAddChildInfoForInvalidRequest() throws Exception {
        try {
            final AddNewChildInfoImpl.Config config = Mockito.mock(AddNewChildInfoImpl.Config.class);
            final Map<String, Object> requestMap = new HashMap<>();
            final Cookie cookie = new Cookie("JSESSIONID", "213123132");
            ...
            requestMap.put(Constant.REQUEST_COOKIES_KEY, cookieObj);
            Mockito.when(config.endPoint())
                    .thenReturn(endPointUrl.replace("PORT_NUMBER", "8080"));
            impl.activate(config);
            impl.addChildInfo(requestMap);
        } catch (final ServiceException serviceException) {
            Assert.assertEquals(Constant.IO_ERROR_KEY, serviceException.getErrorKey());
