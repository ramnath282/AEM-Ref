...
public class DeleteChildInfoImplTest {
    private final String endPointUrl = "http://localhost:PORT_NUMBER/wcs/resources/store/STORE_ID/person/@self/contact/ADD_ID?storeId=STORE_ID&responseFormat=json&updateCookies=true";
    private MockWebServer mockWebServer;
    @Mock
    private PropertyReaderService propertyReaderService;
    @InjectMocks
    private DeleteChildInfoImpl impl;

    @Test
    public void testDeleteChildInfo() throws Exception {
        try {
            final DeleteChildInfoImpl.Config config = Mockito.mock(DeleteChildInfoImpl.Config.class);
            ...
            mockResponse.setBody("{\"addressId\":[\"25538540\"]}");
            ...
            cookie.setVersion(1);
            cookieObj[0] = cookie;
            requestMap.put(Constant.REQUEST_BODY, "{\"nickName\":\"SB_S_25530562\"}");
            ...
            baseResponse = impl.deleteChildInfo(requestMap);
    ...
    public void testDeleteChildInfoForWCSError() throws Exception {
        try {
            final DeleteChildInfoImpl.Config config = Mockito.mock(DeleteChildInfoImpl.Config.class);
            final MockResponse mockResponse = new MockResponse();
            final Map<String, Object> requestMap = new HashMap<>();
            ...
            Mockito.when(propertyReaderService.getCookieDomain("AG")).thenReturn("localhost");
            impl.activate(config);
            impl.deleteChildInfo(requestMap);
    ...
    @Test(expected = ServiceException.class)
    public void testDeleteChildInfoForWCSDown() throws Exception {
        try {
            final DeleteChildInfoImpl.Config config = Mockito.mock(DeleteChildInfoImpl.Config.class);
            final Map<String, Object> requestMap = new HashMap<>();
            final Cookie cookie = new Cookie("JSESSIONID", "213123132");
            ...
            requestMap.put(Constant.REQUEST_COOKIES_KEY, cookieObj);
            Mockito.when(config.endPoint())
                    .thenReturn(endPointUrl.replace("PORT_NUMBER", String.valueOf("41343")));
            Mockito.when(propertyReaderService.getStoreId("AG")).thenReturn("10651");
            Mockito.when(propertyReaderService.getCookieDomain("AG")).thenReturn("localhost");
            impl.activate(config);
            impl.deleteChildInfo(requestMap);
    ...
    @Test(expected = ServiceException.class)
    public void testDeleteChildInfoForInvalidRequest() throws Exception {
        try {
            final DeleteChildInfoImpl.Config config = Mockito.mock(DeleteChildInfoImpl.Config.class);
            final Map<String, Object> requestMap = new HashMap<>();
            final Cookie cookie = new Cookie("JSESSIONID", "213123132");
