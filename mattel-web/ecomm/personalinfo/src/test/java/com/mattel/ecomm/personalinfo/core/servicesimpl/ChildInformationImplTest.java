...
public class ChildInformationImplTest {
    private final String endPointUrl = "http://localhost:PORT_NUMBER/wcs/resources/store/STORE_ID/xchild/getchild?storeId=STORE_ID&updateCookies=true&responseFormat=json";
    private MockWebServer mockWebServer;
    @Mock
    private PropertyReaderService propertyReaderService;
    @InjectMocks
    private ChildInformationImpl impl;

    @Test
    public void testGetChildrenInfo() throws Exception {
        try (InputStream is = getClass().getResourceAsStream("get_child_info_response.json")) {
            final ChildInformationImpl.Config config = Mockito.mock(ChildInformationImpl.Config.class);
            final MockResponse mockResponse = new MockResponse();
            final Map<String, Object> requestMap = new HashMap<>();
            final Cookie cookie = new Cookie("JSESSIONID", "213123132");
            final Cookie[] cookieObj = new Cookie[1];
            ChildInformationResponse childInformationResponse;
            ...
            childInformationResponse = impl.getChildrenInfo(requestMap);

            Assert.assertNotNull(childInformationResponse);
            Assert.assertNull(childInformationResponse.getErrors());
            Assert.assertEquals(6, childInformationResponse.getCookieList().size());
            Assert.assertNotNull(childInformationResponse.getChildInformation());
            Assert.assertEquals(4, childInformationResponse.getChildInformation().size());
        } finally {
    ...
    public void testGetChildrenInfoForWCSDown() throws Exception {
        try {
            final ChildInformationImpl.Config config = Mockito.mock(ChildInformationImpl.Config.class);
            final Map<String, Object> requestMap = new HashMap<>();
            final Cookie cookie = new Cookie("JSESSIONID", "213123132");
            ...
            requestMap.put(Constant.REQUEST_COOKIES_KEY, cookieObj);
            Mockito.when(config.endPoint())
                    .thenReturn(endPointUrl.replace("PORT_NUMBER", String.valueOf("8000")));
            Mockito.when(propertyReaderService.getStoreId("AG")).thenReturn("10651");
            Mockito.when(propertyReaderService.getCookieDomain("AG")).thenReturn("localhost");
            impl.activate(config);
            impl.getChildrenInfo(requestMap);
    ...
    @Test(expected=ServiceException.class)
    public void testGetChildrenInfoForWCSError() throws Exception {
        try {
            final ChildInformationImpl.Config config = Mockito.mock(ChildInformationImpl.Config.class);
            final MockResponse mockResponse = new MockResponse();
            final Map<String, Object> requestMap = new HashMap<>();
