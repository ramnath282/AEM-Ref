...
public class UpdateDefaultAddressImplTest {
	...
	@Ignore
	@Test
	public void testForUpdateDefaultAddress() throws IOException, ServiceException {
		try (InputStream is1 = getClass().getResourceAsStream("update_defaultaddress_request.json");
				InputStream is2 = getClass().getResourceAsStream("update_defaultaddress_response.json")) {
			final UpdateDefaultAddressImpl.Config config = Mockito.mock(UpdateDefaultAddressImpl.Config.class);
			...
			baseResponse = impl.updateDefaultAddress(requestHeader);
	...
	public void testUpdateDefaultAddressForWCSDown() throws IOException, ServiceException {
		try (InputStream is = getClass().getResourceAsStream("add_defaultaddress_request.json")) {
			...
			Mockito.when(propertyReaderService.getCookieDomain("AG")).thenReturn("localhost");
			impl.activate(config);
			impl.updateDefaultAddress(requestHeader);
	...
	@Test(expected = ServiceException.class)
	public void testUpdateDefaultAddressForWCSError() throws IOException, ServiceException {
		try (InputStream is = getClass().getResourceAsStream("add_defaultaddress_request.json")) {
			final UpdateDefaultAddressImpl.Config config = Mockito.mock(UpdateDefaultAddressImpl.Config.class);
			final MockResponse mockResponse = new MockResponse();
			final Map<String, Object> requestHeader = new HashMap<>();
	...
	@Test(expected = ServiceException.class)
	public void testUpdateDefaultAddressForInvalidRequest() throws IOException, ServiceException {
		try {
			final UpdateDefaultAddressImpl.Config config = Mockito.mock(UpdateDefaultAddressImpl.Config.class);
			final Map<String, Object> requestHeader = new HashMap<>();
			final Cookie cookie = new Cookie("JSESSIONID", "213123132");
