...
public class UpdateDefaultShippingImplTest {

	private final String endPointUrl = "http://localhost:PORT_NUMBER/wcs/resources/store/STORE_ID/person/@self/checkoutProfile?updateCookies=true";
	private MockWebServer mockWebServer;
	@Mock
	private PropertyReaderService propertyReaderService;
	@InjectMocks
	UpdateDefaultShippingImpl impl;
	...
	@Test
	public void testForUpdateDefaultShipping() throws IOException, ServiceException {
		try (InputStream is1 = getClass().getResourceAsStream("update_shippingmethod_request.json");
				InputStream is2 = getClass().getResourceAsStream("update_shippingmethod_response.json")) {
			final UpdateDefaultShippingImpl.Config config = Mockito.mock(UpdateDefaultShippingImpl.Config.class);
			...
			baseResponse = impl.updateDefaultShipping(requestHeader);
	 ...
	 @Test(expected = ServiceException.class)
	    public void testUpdateDefaultShippingForWCSDown() throws IOException, ServiceException {
	        try (InputStream is = getClass().getResourceAsStream("add_defaultaddress_request.json")) {
	            ...
	            Mockito.when(propertyReaderService.getCookieDomain("AG")).thenReturn("localhost");
	            impl.activate(config);
	            impl.updateDefaultShipping(requestHeader);
	    ...
	    @Test(expected = ServiceException.class)
	    public void testUpdateDefaultShippingForWCSError() throws IOException, ServiceException {
	        try (InputStream is = getClass().getResourceAsStream("add_defaultaddress_request.json")) {
	            final UpdateDefaultShippingImpl.Config config = Mockito.mock(UpdateDefaultShippingImpl.Config.class);
	            final MockResponse mockResponse = new MockResponse();
	            final Map<String, Object> requestHeader = new HashMap<>();
	    ...
	    @Test(expected = ServiceException.class)
	    public void testUpdateDefaultShippingForInvalidRequest() throws IOException, ServiceException {
	        try {
	            final UpdateDefaultShippingImpl.Config config = Mockito.mock(UpdateDefaultShippingImpl.Config.class);
	            final Map<String, Object> requestHeader = new HashMap<>();
	            final Cookie cookie = new Cookie("JSESSIONID", "213123132");
