...
public class MiniCartImplTest {
	...
	@InjectMocks
	private MiniCartImpl miniCartImpl;

	@Test
	public void testfetch() throws IOException, ServiceException {
		try (InputStream is = getClass().getResourceAsStream("minicart_response.json")) {
			final MiniCartImpl.Config config = Mockito.mock(MiniCartImpl.Config.class);
			...
			miniCartImpl.activate(config);
			miniCartResponse = miniCartImpl.fetch(requestMap);
			Assert.assertNotNull(miniCartResponse);
			Assert.assertEquals("7", miniCartResponse.getCartQuantity());
			Assert.assertEquals("SILVER", miniCartResponse.getCustomerSegment());
		} finally {
	...
	public void testfetchForWCSError() throws IOException, ServiceException {
		try {
			final MiniCartImpl.Config config = Mockito.mock(MiniCartImpl.Config.class);
			final MockResponse mockResponse = new MockResponse();
			final Map<String, Object> requestMap = new HashMap<>();
	...
	@Test(expected = ServiceException.class)
	public void testfetchForWCSDown() throws IOException, ServiceException {
		try {
			final MiniCartImpl.Config config = Mockito.mock(MiniCartImpl.Config.class);
			final MockResponse mockResponse = new MockResponse();
			final Map<String, Object> requestMap = new HashMap<>();
