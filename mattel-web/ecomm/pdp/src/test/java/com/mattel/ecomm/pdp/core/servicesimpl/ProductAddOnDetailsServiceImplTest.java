...
public class ProductAddOnDetailsServiceImplTest {
  ...
  @InjectMocks
  private ProductAddOnDetailsServiceImpl impl;
  private MockWebServer mockWebServer;

  @Test
  public void testfetch() throws IOException, ServiceException {
    try (InputStream is2 = getClass().getResourceAsStream("product_add_on_service_response.json")) {
      final ProductAddOnDetailsServiceImpl.Config config = Mockito
          .mock(ProductAddOnDetailsServiceImpl.Config.class);
      final MockResponse mockResponse = new MockResponse();
      final Map<String, Object> requestMap = new HashMap<>();
      final Cookie cookie = new Cookie("JSESSIONID", "213123132");
      final Cookie[] cookieObj = new Cookie[1];
      mockWebServer = new MockWebServer();
      final ProductAddOnResponse productAddOnResponse;
      ...
      cookie.setVersion(1);
      cookieObj[0] = cookie;
      requestMap.put(Constant.PART_NUMBER, "frh89");
      ...
      Mockito.when(propertyReaderService.getCookieDomain("ag_en")).thenReturn("localhost");
      impl.activate(config);
      productAddOnResponse = impl.fetch(requestMap);
      Assert.assertNotNull(productAddOnResponse);
    } finally {
