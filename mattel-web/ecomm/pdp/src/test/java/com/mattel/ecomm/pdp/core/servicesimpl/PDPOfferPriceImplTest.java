...
public class PDPOfferPriceImplTest {

	private final String endPointUrl = "http://localhost:PORT_NUMBER/wcs/resources/store/STORE_ID/price?q=byPartNumbers&partNumber=PART_NUMBER&responseFormat=json&updateCookies=true&storeId=STORE_ID";
	private final String resource_id = "https://nandclwcw002tv.corp.mattel.com/wcs/resources/store/10651/price?q=byPartNumbers&partNumber=FMD27&responseFormat=json&updateCookies=true&storeId=10651";
	private MockWebServer mockWebServer;
	@Mock
	private PropertyReaderService propertyReaderService;
	private PDPOfferPriceResponse pDPOfferPriceResponse;
	@InjectMocks
	private PDPOfferPriceImpl pDPOfferPriceImpl;

	@Test
	public void testGetOfferPrice() throws IOException, ServiceException {
		try (InputStream is = getClass().getResourceAsStream("get_pdp_offer_price_response.json")) {
			final PDPOfferPriceImpl.Config config = Mockito.mock(PDPOfferPriceImpl.Config.class);
			...
			pDPOfferPriceImpl.activate(config);
			pDPOfferPriceResponse = pDPOfferPriceImpl.getOffertPrice(requestMap);
			Assert.assertNotNull(pDPOfferPriceResponse);
			Assert.assertNotNull(pDPOfferPriceResponse.getEntitledPrice());
			Assert.assertEquals(resource_id, pDPOfferPriceResponse.getResourceId());
		} finally {
	...
	public void testGetOfferPrice_WCSERROR() throws IOException, ServiceException {
		try {
			final PDPOfferPriceImpl.Config config = Mockito.mock(PDPOfferPriceImpl.Config.class);
			final MockResponse mockResponse = new MockResponse();
			final Map<String, Object> requestMap = new HashMap<>();
	...
	@Test(expected = ServiceException.class)
	public void testGetOfferPrice_WCSDown() throws IOException, ServiceException {
		try {
			final PDPOfferPriceImpl.Config config = Mockito.mock(PDPOfferPriceImpl.Config.class);
			final MockResponse mockResponse = new MockResponse();
			final Map<String, Object> requestMap = new HashMap<>();
